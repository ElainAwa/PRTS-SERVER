...
- **区域并行**（`region-parallel`）：所有维度（主世界 / 下界 / 末地）的非玩家实体按区块条带划分为多个区域并行 tick；玩家保持原版单线程 tick 语义（容器菜单、网络、Bukkit 玩家事件留在主线程）。区域数由 `region-count` 配置（2/4/8，默认 4），`region-auto-scale` 按主世界负载自动增减。
...
### 玩家主线程 tick 与区域并行覆盖全维度（v1.0.32）

两项正确性修复，在保持并行的同时保证玩法不变：

- **玩家 tick 留在主线程**：维度并行曾把整个 `ServerLevel`（含玩家）放到 worker 线程 tick，与主线程的容器打开处理竞争，导致箱子/菜单被立即关闭。现在有玩家的维度在主线程 tick，仅无玩家维度在 worker 并行。
- **区域并行覆盖下界与末地**：区域状态改为按维度隔离，实体 tick 在所有维度拆区并行，不再局限于主世界。
...
- 部署：复制 jar 到服务端根目录 → 启动。内嵌 common.jar 内容变化时自动重解包，同名版本重建也会刷新，无需手动清理 `.arclight`
