...
/**
 * 动态可调设置枚举（移植自 ServerCore DynamicSetting）。
 * 去掉 Spark 依赖；配置读取改走 PRTS 的 ServerCoreConfig。
 * 1.20.1 无 mob_spawning 包，故移除 MOBCAP_PERCENTAGE（其 onChanged 依赖 mob_spawning.IMobCategory）。
 */
public enum DynamicSetting {
    ...
    public static void initDefaultValues(DynamicManager manager) {
        Map<DynamicSetting, Integer> defaultValues = ServerCoreConfig.dynamic().defaultValues();
