...
package io.izzel.arclight.common.optimization.chunksystem.scheduler;
...
/**
 * 优先级执行器（FlowSched ExecutorManager 移植）：64 级全局优先级队列 +
 * 锁令牌排队（{@link #tryLock(Task)} 冲突任务挂 listener，释放时重新入队）+
 * 固定 worker 线程数。
 *
 * <p>PRTS 适配点：listener 列表改用 JDK ArrayList（去 fastutil 依赖）；
 * 断言改 IllegalStateException（去 FlowSched Assertions 依赖）。
 */
public class ExecutorManager {
    ...
    void releaseLocks(Task task) {
        ...
        for (LockToken token : task.lockTokens()) {
            ...
            if (listeners != null) {
                ...
                } else if (expectedListeners != listeners) {
                    throw new IllegalStateException("Inconsistent lock listeners");
                }
            } else {
    ...
    private void schedule0(Task task) {
        ...
        ChunkSystemStats.sampleQueueDepth(this.globalWorkQueue.size());
    ...
    private static class FreeableTaskList extends ArrayList<Task> {
