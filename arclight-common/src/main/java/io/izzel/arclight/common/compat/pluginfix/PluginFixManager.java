package io.izzel.arclight.common.compat.pluginfix;
...
public class PluginFixManager {
    ...
    public static byte[] injectPluginFix(String plugin, String className, byte[] clazz) {
        ...
        Consumer<ClassNode> patcher = switch (className) {
            ...
            case "com.sk89q.worldedit.bukkit.BukkitConfiguration" -> node -> {
                helloWorld(node, "I accept that I will receive no support with this flag enabled.", MODID);
                helloWorld(node, "allow-editing-on-unsupported-versions", MODID);
                helloWorld(node, "false", MODID);
            };
            case "com.sk89q.worldedit.bukkit.adapter.impl.v1_21.PaperweightAdapter",
                 "com.sk89q.worldedit.bukkit.adapter.ext.fawe.v1_21_R1.PaperweightAdapter" ->
                    node -> helloWorld(node, "org.spigotmc.WatchdogThread", MODID);
            case "cn.lunadeer.dominion.utils.Misc" ->
                    node -> helloWorld(node, "io.papermc.paper.threadedregions.scheduler.ScheduledTask", MODID);
    ...
    private static void removePaper0(ClassNode node) {
        helloWorld(node, "com.destroystokyo.paper.PaperConfig", MODID);
        helloWorld(node, "io.papermc.paper.configuration.Configuration", MODID);
    }
