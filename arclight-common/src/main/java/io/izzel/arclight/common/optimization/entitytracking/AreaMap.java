package io.izzel.arclight.common.optimization.entitytracking;
...
public class AreaMap<T> {
    ...
    public void add(T object, int x, int z, int rawViewDistance) {
        ...
        lastCenters.put(object, ChunkPos.asLong(x, z));
    ...
    public void remove(T object) {
        ...
        final int x = ChunkPos.getX(lastCenter);
        final int z = ChunkPos.getZ(lastCenter);
    ...
    public void update(T object, int x, int z, int rawViewDistance) {
        ...
        final long oldCenter = lastCenters.replace(object, ChunkPos.asLong(x, z));
        final int oldX = ChunkPos.getX(oldCenter);
        final int oldZ = ChunkPos.getZ(oldCenter);
    ...
    // 诊断用：AreaMap 内部占用的 chunk 桶数量（cell 数），用于排查 spread 半径异常膨胀
    public int cellCount() {
        return this.map.size();
    }

    private void updateAdds(T object, int oldX, int oldZ, int oldViewDistance, int newX, int newZ,
            int newViewDistance) {
    ...
    private void updateRemovals(T object, int oldX, int oldZ, int oldViewDistance, int newX, int newZ,
            int newViewDistance) {
    ...
    private void add0(int xx, int zz, T object, boolean notifyListeners) {
        final RawObjectLinkedOpenIdentityHashSet<T> set = map.computeIfAbsent(ChunkPos.asLong(xx, zz), allocHashSet);
        set.add(object);
    ...
    private void remove0(int xx, int zz, T object, boolean notifyListeners) {
        final long coordinateKey = ChunkPos.asLong(xx, zz);
        final RawObjectLinkedOpenIdentityHashSet<T> set = map.get(coordinateKey);
        if (set == null)
    ...
    private void notifyListenersSorted(T object, int x, int z, Listener<T> addListener, LongArrayList set) {
        set.sort(LongComparators.asLongComparator(Comparator
                .comparingLong(l -> chebyshevDistance(x, z, ChunkPos.getX(l), ChunkPos.getZ(l)))));
        final LongIterator iterator = set.iterator();
        while (iterator.hasNext()) {
            final long pos = iterator.nextLong();
            addListener.accept(object, ChunkPos.getX(pos), ChunkPos.getZ(pos));
        }
    }
        ...
        if (viewDistance < 0) {
            for (Long2ObjectMap.Entry<RawObjectLinkedOpenIdentityHashSet<T>> entry : map.long2ObjectEntrySet()) {
                if (entry.getValue().contains(object))
                    throw new IllegalStateException("Unexpected %s in %s ([%d,%d])".formatted(object, entry.getValue(),
                            ChunkPos.getX(entry.getLongKey()), ChunkPos.getZ(entry.getLongKey())));
            }
        } else {
            for (int xx = x - viewDistance; xx <= x + viewDistance; xx++) {
                for (int zz = z - viewDistance; zz <= z + viewDistance; zz++) {
