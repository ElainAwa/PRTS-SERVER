...
public class MemoryOptimizationCleaner {
    ...
    public static void tick() {
        if (!ArclightConfig.spec().getOptimization().isExperimentalOptimizationsEnabled()) return;
        var mem = ArclightConfig.spec().getOptimization().getMemoryOptimization();
        if (!mem.isCacheCleanupEnabled()) return;
        List<String> targets = mem.getCacheCleanupTargets();
        if (targets == null || targets.isEmpty()) {
            if (!started) {
                started = true;
                LOGGER.info("memory-optimization 已启用，但未配置 cache-cleanup-targets，无缓存可清理。");
            }
            return;
        }
        long now = System.currentTimeMillis();
        long intervalMs = (long) mem.getCacheCleanupInterval() * 1000L;
