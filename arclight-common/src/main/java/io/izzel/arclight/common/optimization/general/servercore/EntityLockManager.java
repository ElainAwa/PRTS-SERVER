 ...
 * acquires the lock for a short, non-nested critical section (a couple of
 * map/set operations at most), so no lock ordering issue can arise. The
 * per-{@code EntitySection} element lock lives on the section instance itself
 * (mixin-injected field), keeping the hot entity-move path free of global contention.</p>
 */
