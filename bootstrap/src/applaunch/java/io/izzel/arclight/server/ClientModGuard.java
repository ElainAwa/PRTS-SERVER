...
public final class ClientModGuard {
    ...
    public static synchronized void handleCrash(Throwable t, String[] args) {
        ...
        // v24：崩溃栈顶归属某含客户端代码模组 + 类加载/链接错误（如专用服启动期 sodium 跑 PreLaunchChecks 缺失 org.lwjgl.Version）。
        // 缺失类非 net.minecraft.client 前缀故不命中 direct，需此独立触发路径打通自愈网。
        Path originMod = locateOffendingMod(t);
        boolean clientOrigin = false;
        if (originMod != null && isClassLoadingError(t)) {
            ScanResult r = null;
            try { r = scanJarFull(originMod); } catch (Throwable ignored) {}
            clientOrigin = (r != null && r.hasClient);
        }
        if (!direct && !modLoadFail && !clientOrigin) return; // 无关崩溃，交还调用方正常抛出
        note("CRASH " + (direct ? "CLIENT_DIRECT" : (modLoadFail ? "MOD_LOADING_FAILED" : "CLIENT_ORIGIN")) + " " + summarize(t));
        ...
        // 路径4 (v24)：崩溃栈顶归属某含客户端代码模组（sodium 等在专用服启动期执行客户端专属代码而崩，
        // 如 NoClassDefFoundError org.lwjgl.Version 发生于 PreLaunchChecks；缺失类非 net.minecraft.client 前缀故不命中 direct，需此独立路径）。
        // 与 v14 同款 hasClient 闸门守零误伤：仅当属主模组确含客户端代码才隔离，否则交还正常崩溃。
        if (offenders.isEmpty() && clientOrigin && originMod != null) {
            String modId;
            try { modId = detectModMeta(originMod).modId; } catch (Throwable e) { modId = originMod.getFileName().toString(); }
            offenders.put(modId, new Object[]{originMod, "runtime: 崩溃栈顶归属客户端模组 " + originMod.getFileName()
                + "（专用服启动期执行客户端专属代码，如缺失 org.lwjgl.Version）"});
            note("ORIGIN_SELFHEAL " + originMod.getFileName());
        }
        if (offenders.isEmpty()) {
            if (direct) {
    ...
    public static synchronized void onUncaughtClientFailure(Throwable t) {
        ...
        // v24：与 handleCrash 对齐——仅当属主模组含客户端代码且为类加载/链接错误时才自动隔离，
        // 避免子线程任意异常（如 NullPointerException）误删双端模组；对真实客户端崩溃无回归。
        ScanResult r = null;
        try { r = scanJarFull(mod); } catch (Throwable ignored) {}
        if (r == null || !r.hasClient || !isClassLoadingError(t)) {
            note("UNCAUGHT_SKIP " + (r != null && r.hasClient) + " " + summarize(t));
            return;
        }
        Map<String, Object[]> offenders = new LinkedHashMap<String, Object[]>();
        String modId;
    ...
    /** v24：是否类加载/链接类错误（NoClassDefFoundError / ClassNotFoundException / LinkageError 等）。客户端模组在专用服启动期执行客户端专属代码时多表现为此类错误，是自愈触发的关键特征之一。 */
    static boolean isClassLoadingError(Throwable t) {
        Throwable c = t;
        while (c != null) {
            if (c instanceof LinkageError || c instanceof ClassNotFoundException) return true;
            c = c.getCause();
        }
        return false;
    }
