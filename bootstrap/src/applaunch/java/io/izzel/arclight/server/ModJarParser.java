...
public final class ModJarParser {
    ...
    private static final String SCAN_ALGO_VERSION = "v15"; // v15: 缓存含 clientOnlyMixin/hasAnyServerMixin
    ...
    public static final class ScanResult {
        ...
        public boolean clientOnlyMixin;       // 终判：declaredClientSide || mixinClientOnly
        public boolean hasAnyServerMixin;
        public boolean declaredClientSide;    // v28: 信号 A——mods.toml 核心依赖 side 全 CLIENT（壳/ JiJ 硬证据）
        public boolean mixinClientOnly;       // v28: 信号 B——根级 mixin 仅 client 段（跳过空配置、不递归 JiJ）
    }
    ...
    static List<String> prescanSideSignals(JarFile jf, ScanResult r) {
        ...
        try {
            ...
            while (en.hasMoreElements()) {
                ...
                } catch (IOException ex) { GuardConfig.note("PRESCAN_ENTRY_FAIL " + e.getName() + " " + ex); }
            }
