...
| **NearbyPlayerIndex (NPI)** | `[Luminara-NPI]` | 空间化最近玩家索引（源自 HariPlayer/routeB），供优化逻辑查询附近玩家 |
| **动力铁轨优化** | `[Luminara-PoweredRails]` | 限定传播深度的铁轨供电路径搜索（`PoweredRailsOptimized`）；开关 `optimize-powered-rails`，默认开启 |
